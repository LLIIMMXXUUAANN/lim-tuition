...
- **`features/students/components/StudentForm`** — on Save, calls `/api/students/{id}` PUT (update) or `/api/students` POST (create). The backend handles Google Calendar sync automatically when `class_schedule` changes and the student already has `calendar_event_ids` (nuke-and-repave + Drive Meet doc rewrite). If the response contains `googleWarning`, the form stays open displaying the amber warning — they close via ← Cancel. "Remove Student" opens a confirmation dialog that calls `/api/students/{id}` DELETE; the backend trashes the Drive folder and deletes Calendar events. Delete response contains `driveError` / `calendarError` fields; Google cleanup failure shows an in-dialog amber warning but doesn't block the DB deletion. Meet and Drive links are shown as read-only `ExternalLink` displays in the form if set (not editable here — set via the Sync All button or the agent's `sync_all_students` tool). **Double-submit guard:** `handleSubmit` checks/sets an `isSubmittingRef` (`useRef`) before touching any state — refs mutate synchronously, unlike `setSaving(true)` which is batched and doesn't disable the button until the next render. This closes the race where a second click lands before React commits the `disabled` attribute (most likely on POST, since PUT is idempotent by row `id` anyway). The ref resets in a `finally` block so a failed submit doesn't lock the form. **Idempotency-Key on create:** the POST branch also sends a client-generated `Idempotency-Key` header (`idempotencyKeyRef`, lazily set via `crypto.randomUUID()`), which the backend's atomic RPC (`tuition-api/docs/decisions.md`, "Idempotency-Key") uses to avoid creating a duplicate student row, Calendar event series, or Drive folder on a manual retry after an error. The key rotates in two cases: after a successful response (so a later resubmit from the same mounted form is a new create, not a replay), and proactively before sending if the current payload no longer matches `lastSubmittedPayloadRef` (a `fast-deep-equal` comparison — see `docs/decisions.md`, "Students") — i.e. the user edited the form since the last attempt under the current key, so it's treated as a new request rather than letting the backend reject a stale-key resubmission. All three mutations (create/update/delete) use TanStack Query's `useMutation` — see "Data fetching" below.
- **`features/students/components/SyncAllButton`** — banner at the bottom of the students list; one click syncs all active students' Google Calendar events and Drive Meet docs to match the DB schedule via `POST /api/google/sync-all` (a `useMutation`). Results show per-student status (✓ synced / – skipped / ✗ error), derived from `mutation.data`. If `invalid_grant` is detected, shows a reconnect link. "Dismiss" calls `mutation.reset()` rather than separate local state. All sync logic (Calendar search, nuke-and-repave, Meet link recovery) runs in the backend — see `tuition-api/CLAUDE.md`.
- **`features/templates/components/TemplatesList`** — receives `initialData` and `students` props from the server; renders a 4-tab layout (Payment · Review · Recommendation · First Approach). The Payment tab contains `PaymentGenerator` followed by the payment templates; the other tabs contain their respective templates. `TEMPLATE_META` is imported from `src/shared/lib/templates.ts` — a `Record<string, { title, description }>`, look up by id directly. Template saves call `PUT /api/templates/{id}` via a `useMutation`; save state per card cycles through `idle → saving → saved/error` driven by `mutation.isPending`/`isSuccess`/`isError`, with the "saved"/"error" indicator auto-resetting via `mutation.reset()` on a `setTimeout` (2s success, 3s error) — same behavior as before, now triggered from `onSettled`-adjacent callbacks instead of manual state.
- **`features/templates/components/PaymentGenerator`** — client component rendered inside `TemplatesList`'s Payment tab; calculates session dates and fee from the student's `class_schedule` via `/api/generate-payment` (a `useMutation`, result read from `mutation.data`).
