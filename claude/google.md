...
**Google setup flow:** Google Calendar events and Drive folders are created by running **Sync All** (SyncAllButton) after a student is created, or by asking the agent to run `sync_all_students`. For subsequent schedule changes, saving `StudentForm` calls `PUT /api/students/{id}` — the backend patches Calendar events and rewrites the Drive Meet doc automatically if `calendar_event_ids` exist. Use **Sync All** (SyncAllButton) to repair any student whose Calendar or Drive state is out of sync.
