...
- **`lib/agent/lg/template-agent.ts`** — exports `TEMPLATE_PROMPT` + `makeTemplateAgent(supabase)`: templates/payment-messages subagent; `TEMPLATE_PROMPT` rule 3 enforces ALWAYS calling `generate_payment_message` — NEVER writing payment message content directly ("The tool returns the actual message with real student data")
...
- **`api/agent/lg/chat/route.ts`** — stateless POST handler; accepts `lgHistory?: StoredMessage[]` in the request body; if provided, restores history with `mapStoredMessagesToChatMessages` and appends the new user `HumanMessage` — giving the LLM full tool call context (supervisor messages, tool calls, tool results, handoff pairs) from prior turns; falls back to text-only conversion if absent (first turn); creates a per-request `AbortController` that fires on either client disconnect (`req.signal`) or the stop endpoint (registered in `requestAbortControllers`); calls `makeSupervisor`, streams via `pipeLangGraphStream` with an `onComplete` callback that emits `{ type: 'lg_history', messages }` (full history serialised with `mapChatMessagesToStoredMessages` as `[...messages, ...accumulatedMessages]`) before `done`; emits `{ type: 'stopped' }` in the `finally` block only if a soft-stop was requested, the client is still connected, AND `completedNormally` is `false` — the `completedNormally` guard prevents a spurious "Cancelled" label when stop is clicked at the exact moment a response finishes
...
**Stateless design:** no checkpointer is passed to `.compile()` and no `thread_id` is passed in the stream config. The frontend sends full tool call history on every request (`lgHistory: StoredMessage[]` for LangGraph, `geminiHistory: Content[]` for classic), reconstructed from localStorage. The route emits it back as `{ type: 'lg_history' }` / `{ type: 'history' }` before `done` so the frontend can persist the updated history. The graph's internal state (accumulated messages) lives only for the duration of one HTTP request.
