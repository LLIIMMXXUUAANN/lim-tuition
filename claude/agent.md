...
- **`lib/agent/lg/stream-adapter.ts`** — `pipeLangGraphStream(stream, emit, signal?, requestId?): Promise<boolean>`: translates LangGraph's multi-mode event stream into the same SSE event types as the classic agent (`chunk`, `step`, `done`, `stopped`, `error`, `download_schedule`, `slots_ready`); returns `true` if the stream completed normally (`done` emitted), `false` if aborted or stopped early; uses `AIMessage.isInstance()` throughout (handles both `AIMessage` and `AIMessageChunk`); checks `stopSignals` after each event as a belt-and-suspenders fallback if LangGraph doesn't honour the AbortSignal fast enough
- **`api/agent/lg/chat/route.ts`** — stateless POST handler; creates a per-request `AbortController` that fires on either client disconnect (`req.signal`) or the stop endpoint (registered in `requestAbortControllers`); calls `makeSupervisor`, streams via `pipeLangGraphStream`; emits `{ type: 'stopped' }` in the `finally` block only if a soft-stop was requested, the client is still connected, AND `completedNormally` is `false` — the `completedNormally` guard prevents a spurious "Cancelled" label when stop is clicked at the exact moment a response finishes
