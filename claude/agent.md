...
- Stored messages include `id` (UUID), `role` (`'user'` | `'agent'`), `content`, `steps[]`, and `timestamp` (ISO string, optional — absent on messages stored before this field was added)
...
- **Timestamps:** each message bubble shows a timestamp below it (`text-xs text-slate-400`), right-aligned for user, left-aligned for agent. `formatMessageTime(iso, now)` formats in MYT: time only (`"2:34 PM"`) if today, `"May 20, 2:34 PM"` if earlier this year, `"May 20 2025, 2:34 PM"` if a different year. Options objects (`MYT_DATE_KEY_OPTS`, `MYT_TIME_OPTS`, `MYT_DAY_MONTH_OPTS`) are module-level constants; `renderNow = new Date()` is computed once per render and passed in to avoid N allocations during SSE chunk re-renders.
...
- **LangGraph toggle:** `useLangGraph` lazy-initialised from `localStorage` (`agent_use_lg`); persisted on every change. When on, `send()` routes to `/api/agent/lg/chat` instead of `/api/agent/chat`. Both endpoints emit the same SSE event format so the chat UI handles both identically.
