...
- Stored messages include `id` (UUID), `role` (`'user'` | `'agent'`), `content`, `isError?` (boolean — set when any error path fires; `undefined` on old messages so no button appears), `steps[]`, and `timestamp` (ISO string, optional — absent on messages stored before this field was added)
...
- **SSE streaming:** `send(retryMsgId?: string)` — when called without arguments (normal send), creates a new user message + blank pending agent message and appends both to history. When called with a `retryMsgId`, resets the existing error bubble in-place (`content: ''`, `steps: []`, `isError: false`) and builds `apiMessages` from the history slice up to (not including) that bubble — no user message is added. `toApiMsg` helper maps `ChatMessage → { role, content }` (shared by both paths). `step` events append to the pending message's `steps[]`; `chunk` events append to `content`; all three error paths (SSE error event, `catch`, `finally !received`) set `isError: true`. A `received` flag gates the fallback message.
